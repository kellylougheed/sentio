    ...
    if request.method == "POST":
        try:
            ...
            if not request.form.get("url") and not request.form.get("text"):
                return render_template("error.html", msg="Missing file, URL, or text")

        if not request.form.get("word"):
            return render_template("error.html", msg="Missing word for frequency analysis")
        ...
        if request.form.get("url"):
            ...
            try:
                raw = response.read().decode('utf8').lower() # Lowercase for case insensitivity
            except:
            ...
            try:
                text = request.files["file"].read().decode("utf-8").lower() # Lowercase for case insensitivity
            except:
                return render_template("error.html", msg="Invalid file")
        ...
        words_results = []
        for w in words:
            count = text.count(w.strip().lower()) # Strip whitespace & lowerspace
